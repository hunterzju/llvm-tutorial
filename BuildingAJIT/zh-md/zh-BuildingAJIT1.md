...
- [第2章](BuildingAJIT2.html)：通过添加一个新的层来扩展基本的KaleidoscopeJIT，该层将优化IR和生成的代码。
- [第3章](BuildingAJIT3.html)：通过添加按需编译层来延迟编译IR，从而进一步扩展JIT。
- [第\#4章](BuildingAJIT4.html)：通过将按需编译层替换为直接使用ORC编译回调API以将IR生成推迟到调用函数的定制层，来改进JIT的惰性。
- [第\#5章](BuildingAJIT5.html)：使用JIT远程API将JITing代码添加到权限降低的远程进程中。
...
[下一步：扩展KaleidoscopeJIT](BuildingAJIT2.md)
