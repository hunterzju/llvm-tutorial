...
源代码级别调试使用格式化数据来帮助调试器将二进制代码和计算机状态转换回程序员编写的源代码。在LLVM中，我们通常使用称为[DWARF](http://dwarfstd.org)格式。DWARF是一种表示类型、源位置和变量位置的紧凑编码。
...
DWARF中代码段的顶层容器是编译单元。它包含单个翻译单元的类型和功能数据(读取：一个源代码文件)。因此，我们需要做的第一件事是为fier.ks文件构建一个。
...
这既告诉主`IRBuilder‘我们所在的位置，也告诉我们所在的作用域。作用域可以是编译单元级别的，也可以是最接近的封闭词法block，就像当前函数一样。为了表示这一点，我们创建了一个作用域堆栈：
